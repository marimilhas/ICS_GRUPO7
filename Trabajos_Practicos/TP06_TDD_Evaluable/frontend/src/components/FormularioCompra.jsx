...
const FormularioCompra = ({ onCompra }) => {
  ...
  const obtenerCategoriaEdad = (edad, pase) => {
    ...
    if (edad < 10) return {
      ...
      precio: pase === "regular" ? "$2.500,00" : "$5.000,00"
    };
    if (edad < 61) return {
      tipo: "Adulto",
      ...
      precio: pase === "regular" ? "$5.000,00" : "$10.000,00"
    };
    return {
      tipo: "Adulto Mayor",
  ...
  const calcularTotal = () => {
    ...
    entradas.forEach(e => {
      if (e.edad < 3) {
        // Infantes: gratis
        total += 0;
      } else if (e.edad < 10 || e.edad >= 61) {
        // Niños y Adultos Mayores: 50% descuento
        total += (e.pase === "regular" ? 5000 : 10000) * 0.5;
      } else {
        // Adultos: precio completo
        total += (e.pase === "regular" ? 5000 : 10000);
      }
    });
  ...
  const handleSubmit = (e) => {
    ...
    const compraData = {
      fecha,
      cantidad,
      entradas, // ahora enviamos array de objetos {edad, pase}
      formaPago,
      mail,
      total: calcularTotal()
    };
    ...
    if (onCompra) {
      onCompra(compraData, "¡Su compra de entradas a EcoHarmony Park fue exitosa!");
    }
  };
    ...
    if (!acc[key]) {
      acc[key] = {
        ...
        precioUnitario: parseFloat(
          obtenerCategoriaEdad(entrada.edad, entrada.pase)
            .precio
            .replace('$', '')
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-green-medium">
                  <span className="text-2xl font-bold text-gray-900">$5.000,00</span>
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-green-medium">
                  <span className="text-2xl font-bold text-gray-900">$10.000,00</span>
                  <span className="text-sm text-gray-500 ml-2">por persona</span>
                </div>
              </div>
            </div>
        ...
        <div>
          <button
            ...
            className={`w-full font-bold py-4 rounded-xl shadow-lg transition-all duration-300 transform ${formularioValido
